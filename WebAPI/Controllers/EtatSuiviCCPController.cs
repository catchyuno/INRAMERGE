...
namespace WebAPI.Controllers
{
    ...
    [ApiController]
    public class EtatSuiviCCPController : ControllerBase
    {
        private readonly IConfiguration _configuration;
        public EtatSuiviCCPController(IConfiguration configuration)
        {
        ...
        [HttpPost]
        [Route("ListeDemandes")]
        public IActionResult ListeDemandes(EtatSuiviCCP Demande)
        {
            ...
            using (MySqlConnection myCon = new MySqlConnection(sqlDataSource))
            {
                ...
                {
                    var test = myCon.Query("SELECT * FROM etat_ccp_demandes ORDER BY STATUT, DATE DESC", Demande).ToList();
                    return Ok(test);
                }
                if ((Demande.STATUT != "TOUS") && (Demande.DDP == "" || Demande.DDP == "-------" || Demande.DDP == null))
                {
                    var test = myCon.Query("SELECT * FROM etat_ccp_demandes where STATUT=  @STATUT ORDER BY STATUT, DATE DESC", Demande).ToList();
                    return Ok(test);
                }
                if ((Demande.STATUT != "TOUS") && (Demande.DDP != "" || Demande.DDP != null))
                {
                    var test = myCon.Query("SELECT * FROM etat_ccp_demandes where STATUT=  @STATUT AND DDP=  @DDP ORDER BY STATUT, DATE DESC", Demande).ToList();
                    return Ok(test);
                }
                if ((Demande.STATUT == "TOUS") && (Demande.DDP != "" && Demande.DDP != null))
                {
                    var test = myCon.Query("SELECT * FROM etat_ccp_demandes where DDP=  @DDP ORDER BY STATUT, DATE DESC", Demande).ToList();
                    return Ok(test);
                }
                //myCon.close();
                return Ok();
        ...
        [HttpPost]
        [Route("ListeAgents")]
        public IActionResult ListeAgents(EtatSuiviCCP Demande)
        {
            string sqlDataSource = _configuration.GetConnectionString("myinraAppCon");
            using (MySqlConnection myCon = new MySqlConnection(sqlDataSource))
            {
                ...
                {
                    var test = myCon.Query("SELECT DDP, NOM_PRENOM FROM etat_ccp_demandes GROUP BY DDP, NOM_PRENOM ORDER BY NOM_PRENOM", Demande).ToList();
                    test.Add(new { DDP = "-------", NOM_PRENOM = "TOUS" });
                    return Ok(test);
                }
                if (Demande.STATUT != "TOUS" || Demande.STATUT != null)
                {
                    var test = myCon.Query("SELECT DDP, NOM_PRENOM FROM etat_ccp_demandes where STATUT=  @STATUT GROUP BY DDP, NOM_PRENOM  ORDER BY NOM_PRENOM", Demande).ToList();
                    test.Add(new { DDP = "-------", NOM_PRENOM = "TOUS" });
                    return Ok(test);
                }
                //myCon.close();
        ...
        [HttpPost]
        [Route("upload")]
        public IActionResult getfile(EtatSuiviCCP Demande)
        {
            Byte[] b;
            ...
            System.IO.File.WriteAllBytes(@".\Etats\SORTIE\ETAT_CCP\" + fileoutput + ".pdf", b);
            string sqlDataSource = _configuration.GetConnectionString("myinraAppCon");
            using (MySqlConnection myCon = new MySqlConnection(sqlDataSource))
            {
                string DATE_V = (string)Demande.DATE.ToString("yyyy-MM-dd");
                var t = myCon.Query(@"Update etat_ccp_demandes set STATUT = 'TRAITE' where DATE='" + DATE_V + @"' AND DDP=@DDP  AND DDP_DEMANDEUR=@DDP_DEMANDEUR", Demande).ToList();
            }
            PdfSharp.Pdf.PdfDocument outputDocument = new PdfSharp.Pdf.PdfDocument();
            var filename = @".\Etats\SORTIE\ETAT_CCP\" + fileoutput + ".pdf";
                ...
                using (MySqlConnection myCon = new MySqlConnection(sqlDataSource))
                {
                    ...
                    if (testsignature != 0)
                    {
                        ...
                        foreach (var rows in test)
                        {
                            ...
                            gfx.DrawImage(imagesigne, 0, page.Height - 140, page.Width, 110);
                        }
                    }
                    else
                    {
