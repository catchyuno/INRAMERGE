...
@Component({
  selector: 'app-add_edit_etat_prime',
  templateUrl: './add_edit_etat_prime.component.html',
  styleUrls: ['./add_edit_etat_prime.component.css']
})
export class Add_edit_etat_primeComponent implements OnInit {
  today: Date;
  ...
  refreshAnneeList() {
    ...
    this.sharedService.getPrimeAnneeList(val).subscribe(data =>{
     this.AnneeList = data;
  ...
  addEtatPrime(){
    ...
    this.spinner.show();
    this.service.addEtatPrime(val).subscribe(res =>{
      if (res!="")
      {
        alert(res.toString())
      }
  ...
  downloadfile() {
    ...
    let nom_pdf : any = this.DDP + "_" + this.ANNEE + "_" + this.PRIME + "_" + this.DATE_PDF + " (" + this.sharedService.userInfo.DDP + ")";
    let NOM_FILE: any=".\\Etats\\SORTIE\\ETAT_PRIME\\" + nom_pdf + ".pdf";
    this.sharedService.getdownloadPrimefile(NOM_FILE).subscribe(data =>{
