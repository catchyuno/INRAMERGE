...
@Component({
  selector: 'app-show_etat_domiciliation_gestionnaire',
  templateUrl: './show_etat_domiciliation_gestionnaire.component.html',
  styleUrls: ['./show_etat_domiciliation_gestionnaire.component.css'] 
})
export class Show_etat_domiciliation_gestionnaireComponent implements OnInit {
  p: number = 1;
  MSG:string="";
  CATEGORIE_AGENT: any= []
  EtatDomList:any = [];
  ...
  refreshEtatDomList() {
    ...
    this.sharedService.getEtatDomListGest(val).subscribe(data =>{
      this.EtatDomList = data;
