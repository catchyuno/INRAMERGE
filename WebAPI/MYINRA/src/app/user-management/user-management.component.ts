...
export class UserManagementComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.auth.get('agents').subscribe((data:any)=>{
      this.agents=data;
  ...
  addmode(){
    ...
    this.editable=true;
    this.user={email:'',fullName:null,userName:null,password:null,DDP:null,enabled:1};
  }
  ddps(){
    console.log(this.agentselect);
    this.user.FullName=this.agentselect.nom;
    this.user.Email=this.agentselect.Mail;
    this.user.UserName=this.agentselect.Mail;
    this.user.DDP=this.agentselect.doti;
  ...
  valid(){
    let use;
      ...
      if(this.user.password.length>5 && this.user.password==this.cpassword ){
        use={userName:this.user.userName,DDP:this.user.DDP,enabled:1,access:null,email:this.user.email,fullName:this.user.fullName,id:this.user.id,password:this.user.password};
        if(this.user.id==null ||this.user.id=='' ||this.user.id==undefined){
          this.auth.post('register',this.user).subscribe((data:any)=>{
            if(data.success){
              this.auth.toast().success('Utilisateur ajouter!');
              this.reset();
            }else{
              this.auth.toast().error(data.err);
            }
          })
        }else{
          this.auth.post('update',use).subscribe((data:any)=>{
          if(data.success){
            this.auth.toast().success('Vos modification ont été enregistrer!');
            this.reset();
          }else{
            this.auth.toast().error(data.err);
          }
